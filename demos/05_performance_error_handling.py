...
This demo showcases the performance-oriented features and error handling including:
...
def batch_store_states(memory_system, agent_id: str, states: List[Dict[str, Any]]) -> tuple:
...
def run_performance_test(agent_id: str, memory_system) -> Dict[str, float]:
    ...
    num_states = 100
    states = [
        generate_random_state(agent_id, step)
        for step in range(1, num_states + 1)
    ]
...
def test_error_handling(memory_system) -> Dict[str, bool]:
    ...
    try:
        # Intentionally pass invalid query
