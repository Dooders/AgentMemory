...
class SearchModel:
    ...
    def register_strategy(
        self, strategy: SearchStrategy, make_default: bool = False
    ) -> bool:
    ...
    def unregister_strategy(self, strategy_name: str) -> bool:
        ...
        if strategy_name not in self.strategies:
            logger.warning(
                "Attempted to unregister unknown strategy: %s", strategy_name
            )
            return False
        ...
        if strategy_name == self.default_strategy:
            ...
            if self.strategies:
                ...
                logger.debug(
                    "Set new default search strategy to %s", self.default_strategy
                )
            else:
    ...
    def set_default_strategy(self, strategy_name: str) -> bool:
        ...
        if strategy_name not in self.strategies:
            logger.warning(
                "Attempted to set unknown strategy as default: %s", strategy_name
            )
            return False
    ...
    def search(
        ...
        **kwargs
        ...
        logger.debug(
            "Performing search with strategy %s for agent %s", strategy_name, agent_id
        )
    ...
    def get_available_strategies(self) -> Dict[str, str]:
        ...
        """
        return {
            name: strategy.description() for name, strategy in self.strategies.items()
        }
