...
def generate_test_state(complexity: int = 1) -> Dict[str, Any]:
    ...
    state = {
        ...
        "attributes": {f"attr_{i}": random.random() for i in range(complexity * 10)},
        "goals": [
            {
                "id": f"goal_{i}",
    ...
    **kwargs,
    ...
    for complexity in data_complexity_levels:
        ...
        for batch_size in batch_sizes:
            print(
                f"Testing write throughput with batch_size={batch_size}, complexity={complexity}"
            )
            ...
            temp_db_path = os.path.join(
                tempfile.gettempdir(), f"benchmark_ltm_{complexity}_{batch_size}.db"
            )
            ...
            for i, state in enumerate(test_states):
                memory_system.store_agent_state(
                    agent_id="benchmark_agent", state_data=state, step_number=i
                )
            end_time = time.time()
            ...
            im_throughput = (
                batch_size / (execution_time * 2) if execution_time > 0 else 0
            )
            ...
            batch_results["im"] = {
                ...
                "total_time_seconds": execution_time,
                "items_processed": batch_size
                // 2,  # Approximation of how many would transition
            }
            ...
            execution_time = end_time - start_time
            ltm_throughput = (
                batch_size / (execution_time * 4) if execution_time > 0 else 0
            )
...
def benchmark_read_latency(
    num_samples: int = 100, data_complexity_levels: Optional[List[int]] = None, **kwargs
) -> Dict[str, Any]:
    ...
    for complexity in data_complexity_levels:
        print(
            f"Testing read latency with complexity={complexity}, samples={num_samples}"
        )

        # Create memory system with temp db path
        config = MemoryConfig()
        temp_db_path = os.path.join(
            tempfile.gettempdir(), f"benchmark_read_latency_{complexity}.db"
        )
        config.ltm_config.db_path = temp_db_path
        memory_system = AgentMemorySystem(config)
        ...
        for i in range(5):
            ...
            memory_system.retrieve_by_time_range(
                agent_id="benchmark_agent", start_step=start_step, end_step=end_step
            )
            end_time = time.time()

            stm_latencies.append(
                (end_time - start_time) * 1000 / batch_size
            )  # ms per item
        ...
        for i in range(5):
            ...
            memory_system.retrieve_by_attributes(
                agent_id="benchmark_agent", attributes=query
            )
            end_time = time.time()
        ...
        for i in range(5):
            ...
            memory_system.retrieve_similar_states(
                agent_id="benchmark_agent", query_state=query_state, k=10
            )
            end_time = time.time()
        ...
        tier_results["time_range_retrieval"] = {
            ...
            "average_ms_per_item": (
                sum(stm_latencies) / len(stm_latencies) if stm_latencies else 0
            ),
        }
        ...
        tier_results["attribute_retrieval"] = {
            ...
            "average_ms_total": (
                sum(im_latencies) / len(im_latencies) if im_latencies else 0
            ),
        }
        ...
        tier_results["similarity_retrieval"] = {
            ...
            "average_ms_total": (
                sum(ltm_latencies) / len(ltm_latencies) if ltm_latencies else 0
            ),
        }
...
def benchmark_memory_efficiency(
    data_complexity_levels: Optional[List[int]] = None, **kwargs
) -> Dict[str, Any]:
    ...
    for complexity in data_complexity_levels:
        ...
        config = MemoryConfig()
        temp_db_path = os.path.join(
            tempfile.gettempdir(), f"benchmark_memory_efficiency_{complexity}.db"
        )
        config.ltm_config.db_path = temp_db_path
        memory_system = AgentMemorySystem(config)
