...
class BenchmarkRunner:
    ...
    def __init__(
        ...
        self.results_manager = results_manager or BenchmarkResults(
            self.config.output_dir
        )
        self.running_benchmarks: Set[str] = set()
        self.completed_benchmarks: Dict[str, str] = (
            {}
        )  # Maps benchmark ID to result file path
        ...
        for cat in categories:
            ...
            try:
                ...
                for name in dir(module):
                    if name.startswith("benchmark_") and callable(
                        getattr(module, name)
                    ):
    ...
    def run_benchmark(
        ...
        try:
            ...
            if not hasattr(module, func_name) or not callable(
                ...
                raise ValueError(
                    f"Benchmark function not found: {func_name} in {module_path}"
                )
            ...
            logger.info(
                f"Benchmark {benchmark_id} completed in {execution_time:.2f} seconds"
            )
    ...
    def run_category(
        ...
        if parallel and len(benchmark_funcs) > 1:
            logger.info(
                f"Running {len(benchmark_funcs)} benchmarks in category {category} in parallel"
            )
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_to_benchmark):
                    ...
                    try:
                        ...
                        logger.error(
                            f"Benchmark {category}.{benchmark_name} failed: {str(e)}"
                        )
        else:
            logger.info(
                f"Running {len(benchmark_funcs)} benchmarks in category {category} sequentially"
            )
    ...
    def run_all(
        ...
        if parallel_categories and len(all_benchmarks) > 1:
            logger.info(
                f"Running {len(all_benchmarks)} benchmark categories in parallel"
            )
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_category = {
                    executor.submit(
            ...
            logger.info(
                f"Running {len(all_benchmarks)} benchmark categories sequentially"
            )
            for category in all_benchmarks:
                try:
                    category_results = self.run_category(
                        category, params.get(category), parallel_benchmarks, max_workers
                    )
                    results[category] = category_results
                except Exception as e:
                    logger.error(f"Category {category} failed: {str(e)}")
        ...
        for category, result_files in current_results.items():
            ...
            for result_file in result_files:
                ...
                for metric, current_value in flat_current.items():
                    if metric in flat_baseline and isinstance(
    ...
    def generate_comparison_report(
        self, comparison: Dict[str, Dict[str, Any]], output_dir: Optional[str] = None
    ) -> str:
            ...
            for benchmark, metrics in benchmarks.items():
                ...
                for metric, values in metrics.items():
                    ...
                    row_class = (
                        "regression"
                        if is_regression
                        else ("improvement" if abs(rel_change) > 5 else "")
                    )
