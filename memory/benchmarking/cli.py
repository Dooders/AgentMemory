...
def parse_args() -> argparse.Namespace:
    ...
    run_parser.add_argument(
        "--category", help="Benchmark category to run (e.g., 'storage', 'retrieval')"
    )
    run_parser.add_argument("--benchmark", help="Specific benchmark to run")
    run_parser.add_argument("--config", help="Path to configuration file")
    run_parser.add_argument("--output-dir", help="Directory to store results")
    run_parser.add_argument(
        "--params", help="JSON string or file path with parameters to override"
    )
    run_parser.add_argument(
    ...
    list_parser.add_argument("--category", help="Benchmark category to list")

    # Results command
    results_parser = subparsers.add_parser(
        "results", help="Work with benchmark results"
    )
    results_subparsers = results_parser.add_subparsers(
        dest="results_command", help="Results command"
    )

    # Results list command
    results_list_parser = results_subparsers.add_parser(
        "list", help="List available results"
    )
    results_list_parser.add_argument("--category", help="Filter by category")
    results_list_parser.add_argument("--benchmark", help="Filter by benchmark name")
    results_list_parser.add_argument(
        "--results-dir", help="Directory containing results"
    )

    # Results report command
    results_report_parser = results_subparsers.add_parser(
        "report", help="Generate a results report"
    )
    results_report_parser.add_argument("--category", help="Filter by category")
    results_report_parser.add_argument("--benchmark", help="Filter by benchmark name")
    results_report_parser.add_argument(
        "--results-dir", help="Directory containing results"
    )
    results_report_parser.add_argument(
        "--output-dir", help="Directory to save the report"
    )

    # Compare command
    compare_parser = subparsers.add_parser("compare", help="Compare benchmark results")
    compare_parser.add_argument(
        "--current-dir", required=True, help="Directory containing current results"
    )
    compare_parser.add_argument(
        "--baseline-dir", required=True, help="Directory containing baseline results"
    )
    compare_parser.add_argument(
        "--threshold",
        type=float,
    ...
    compare_parser.add_argument(
        "--output-dir", help="Directory to save the comparison report"
    )
    ...
    try:
        if args.category and args.benchmark:
            ...
            result_paths = runner.run_category(
                args.category, params, args.parallel, args.workers
            )
            ...
            results = runner.run_all(
                category_params, parallel_categories, parallel_benchmarks, args.workers
            )

            total_benchmarks = sum(len(paths) for paths in results.values())
            print(
                f"Completed {total_benchmarks} benchmarks across {len(results)} categories"
            )
    ...
    try:
        if args.results_command == "list":
            ...
            if not result_files:
                if args.category and args.benchmark:
                    print(
                        f"No results found for category '{args.category}' and benchmark '{args.benchmark}'"
                    )
                elif args.category:
...
def compare_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        comparison = runner.compare_with_baseline(
            current_results, args.baseline_dir, args.threshold
        )
        ...
        report_path = runner.generate_comparison_report(comparison, args.output_dir)
        ...
        for category, benchmarks in comparison.items():
            for benchmark, metrics in benchmarks.items():
                regressions = sum(
                    1 for m in metrics.values() if m.get("is_regression", False)
                )
                if regressions > 0:
                    has_regressions = True
                    print(
                        f"WARNING: {category}.{benchmark} has {regressions} regression(s)"
                    )
