...
class MemoryImporter:
    ...
    def import_memories(self, agent_id: int) -> List[MemoryMetadata]:
        ...
        with self.db_manager.session() as session:
            ...
            agent = (
                session.query(self.db_manager.AgentModel)
                .filter(self.db_manager.AgentModel.agent_id == agent_id)
                .first()
            )
            ...
            if self.config.total_steps is None:
                max_step = session.query(
                    func.max(self.db_manager.SimulationStepModel.step_number)
                ).scalar()
                if max_step is not None:
                    ...
                    logger.warning(
                        "Could not determine total steps, using step number as current step"
                    )
            ...
            for model_name in self.memory_type_mapper.required_models:
                ...
                try:
                    model_memories = self._import_memory_type(
                        session, agent_id, model_name
                    )
                    logger.debug(
                        f"Found {len(model_memories)} memories for {model_name}"
                    )
    ...
    def _import_memory_type(
        self, session, agent_id: int, model_name: str
    ) -> List[MemoryMetadata]:
        """
            ...
            for memory in batch:
                try:
                    memory_metadata = self._import_memory(
                        memory, memory_type, model_name
                    )
                    if memory_metadata:  # Only add if memory was successfully created
                        memories.append(memory_metadata)
                        logger.debug(
                            f"Created memory for {model_name} with memory_id {memory_metadata.memory_id}"
                        )
                except Exception as e:
        ...
        logger.info(
            f"Successfully imported {len(memories)} memories of type {model_name} for agent {agent_id}"
        )
        return memories
    ...
    def _get_memory_query(self, session, model, agent_id: int):
        ...
        logger.debug(
            f"SQL Query for {model.__name__}: {query.statement.compile(compile_kwargs={'literal_binds': True})}"
        )

        # Log the count of results
        count = query.count()
        logger.debug(
            f"Found {count} records for {model.__name__} with agent_id {agent_id}"
        )
    ...
    def _import_memory(
        self, memory: Any, memory_type: str, model_name: str
    ) -> MemoryMetadata:
        """
        ...
        if step_number is None:
            logger.warning(
                f"No step_number found for {model_name} with id {getattr(memory, 'id', 'unknown')}"
            )
            return None

        # Get agent_id and ensure it's not None
        agent_id = getattr(memory, "agent_id", None)
        if agent_id is None:
            logger.warning(
                f"No agent_id found for {model_name} with id {getattr(memory, 'id', 'unknown')}"
            )
            return None
        ...
        metadata = {
            ...
            "agent_id": str(agent_id),  # Ensure agent_id is string
            "position": {
            ...
            metadata.update(
                {
                    ...
                    "resources_transferred": getattr(
                        memory, "resources_transferred", 0.0
                    ),
        ...
        # Generate memory ID using the new helper function
        ...
        return MemoryMetadata(
            memory_id=memory_id,
            agent_id=str(agent_id),  # Ensure agent_id is string
            memory_type=str(memory_type),  # Ensure memory_type is string
            step_number=step_number,
            tier=str(tier),  # Ensure tier is string
            metadata=metadata,
    ...
    def _handle_import_error(self, error: Exception, agent_id: int, model_name: str):
        """
