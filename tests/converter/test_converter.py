...
def mock_agent_importer():
    ...
    importer = MagicMock()
    importer.import_agents.return_value = [
...
def test_from_agent_farm_successful_import(tmp_path, config, mock_db_manager, mock_agent_importer, mock_memory_importer):
...
def test_from_agent_farm_validation_skip(tmp_path, config, mock_db_manager):
    ...
    with patch('converter.converter.DatabaseManager', return_value=mock_db_manager), \
         patch('converter.converter.AgentImporter'), \
         patch('converter.converter.MemoryImporter'):
        
        memory_system = from_agent_farm(str(db_path), config)
        assert isinstance(memory_system, AgentMemorySystem)

def test_from_agent_farm_memory_import_error(tmp_path, config, mock_db_manager, mock_agent_importer):
...
def test_from_agent_farm_import_verification(tmp_path, config, mock_db_manager, mock_agent_importer, mock_memory_importer):
