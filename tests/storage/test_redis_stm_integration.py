    ...
    return [
        {
            ...
            "embeddings": {"full_vector": [0.1 * i, 0.2 * i, 0.3 * i]},
        }
...
def test_get_by_importance_integration(stm_store, memory_entries):
    ...
    for entry in memory_entries:
        ...
        print(
            f"Storing memory {entry['memory_id']} with importance score: {entry['metadata']['importance_score']}"
        )
        stm_store.store(agent_id, entry)
    ...
    importance_data = stm_store.redis.client.zrange(
        importance_key, 0, -1, withscores=True
    )
