...
def test_redis_vector_index_init(mock_redis_client):
    ...
    index = RedisVectorIndex(mock_redis_client, index_name="test_index", dimension=128)
...
def mock_indices():
    ...
    for index in [stm_index, im_index, ltm_index]:
        ...
        index.search.return_value = [
            {"id": "test1", "score": 0.9, "metadata": {"type": "test"}}
        ]
...
def test_vector_store_store_memory_vectors():
    ...
    # Test STM tier
    result = store.store_memory_vectors(memory_entry, tier="stm")
    assert result is True
    ...
    # Test IM tier
    result = store.store_memory_vectors(memory_entry, tier="im")
    assert result is True
    store.im_index.add.assert_called_once_with("test1", [0.1] * 384, {"type": "test"})
    store.stm_index.reset_mock()
    store.im_index.reset_mock()
    store.ltm_index.reset_mock()

    # Test LTM tier
    result = store.store_memory_vectors(memory_entry, tier="ltm")
    assert result is True
    store.ltm_index.add.assert_called_once_with("test1", [0.1] * 384, {"type": "test"})
