...
def create_embeddings(
    states: List[Dict[str, Any]], engine: TextEmbeddingEngine
) -> np.ndarray:
    """
...
def test_state_transitions(
    states: List[Dict[str, Any]], embeddings: np.ndarray
) -> None:
    """
...
def test_agent_clustering(
    states: List[Dict[str, Any]], embeddings: np.ndarray, agent_ids: List[str]
) -> None:
    """
...
def visualize_embeddings(states: List[Dict[str, Any]], embeddings: np.ndarray) -> None:
    ...
    for agent, points in agent_points.items():
        ...
        for idx, step in to_label:
            plt.annotate(
                f"{step}", (embeddings_2d[idx, 0], embeddings_2d[idx, 1]), fontsize=8
            )
...
def test_position_similarity(
    ...
    # Extract positions
    positions = np.array(
        [[state["position_x"], state["position_y"]] for state in states]
    )
    ...
    for i in range(len(positions)):
        for j in range(len(positions)):
            position_distances[i, j] = np.sqrt(
    ...
    sample_indices = np.random.choice(
        len(states), size=min(1000, len(states)), replace=False
    )
    ...
    print(
        f"Correlation between position distance and embedding similarity: {correlation:.4f}"
    )
